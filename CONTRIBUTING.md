...
apps/<slug>/icon.*, cover.png, screenshots/, MEDIA.md   optional images and their sources
featured.json, featured/       the sponsored slot (empty until there is a sponsor)
...
.github/workflows/stats.yml    hourly: rebuild stats.json and redeploy Pages, no commit
...
Use only images the upstream project publishes: its logo, its social preview or
banner, the screenshots in its README or docs. Never add generated or drawn art,
and leave an image out when the upstream licence does not cover it or it shows
other companies' logos. An entry without an icon is shown with a monogram the
manager draws from its name, and one without a cover or screenshots simply shows
none. Scaling, converting to PNG, or rendering an upstream SVG to PNG is fine;
say so in `MEDIA.md`.

Every image must be named in `MEDIA.md` on a line that links the upstream file
at a pinned commit, with its licence and what was changed. `pnpm validate`
refuses an image that `MEDIA.md` does not name, or names without a link. It helps
to note why an upstream image was left out, so no one adds it later. A
screenshot's alt text comes from its file name: `01-inbox-view.png` is read as
"Inbox view". SVGs may not contain scripts, event handlers, `foreignObject`, or
links to other files.

The images are published on the Pages site at the same paths, and `index.json`
lists each one by URL and sha256. The manager shows an image only when it comes
from the catalog site and its bytes match that digest, and it serves the image to
its users itself, so their browsers never contact the catalog site.
...
The catalog has had one sponsored slot since its first release, so sponsorship was
part of Appflare's design from the start, not added later. `featured.json` holds
its items and is an empty array while there is no sponsor; `index.json` always
carries the `featured` array, empty or not.

An item can promote anything, an app in the catalog or not. The manager shows it
on its catalog page only, one item at a time, always labelled "Sponsored" (the
label is part of the manager, so no entry can remove it). Every user can hide an
item for themselves. Sponsored items never change search results, sort order,
popularity, install checks, or what gets installed, and nothing about them (views,
clicks, hides) is sent anywhere. Links open with `rel="sponsored noopener
noreferrer"`, so the sponsor never learns which manager a visitor came from, and
nothing is added to the URL.

To add an item, open a pull request that appends it to `featured.json`:

```jsonc
{
  "id": "acme-2026-10",                 // lowercase, never reused; a new campaign gets a new id
  "title": "Acme Edge",                  // up to 60 characters
  "text": "Deploy to 300 cities.",       // plain text, up to 200 characters
  "sponsor": { "name": "Acme", "url": "https://acme.example" },
  "image": { "alt": "The Acme Edge dashboard" },  // optional; the file is featured/<id>.png
  "link": { "url": "https://acme.example/edge", "label": "Learn more" },
  "slug": "cut",                         // optional: an app in the catalog this item promotes
  "startsAt": "2026-10-01T00:00:00Z",    // optional window, UTC
  "endsAt": "2026-11-01T00:00:00Z"
}
```

An item needs a `link` (https only) or a `slug`, or both. Its image, when it has one,
is a 1200x630 PNG at `featured/<id>.png`: it is published on the catalog site and
pinned by its sha256 in `index.json`, so the manager never loads anything from a
sponsor's servers. `pnpm validate` checks the items, the images, and that every
`slug` exists. Items are shown in file order; the first one inside its window that
a user has not hidden is the one they see. The catalog's own maintainers review
every change to `featured.json` and `featured/`.

## Popularity

The Pages site serves `stats.json` next to `index.json` (whose `stats` field holds
its URL). Per app it lists the GitHub stars of the upstream repository and, from the
anonymous events Appflare managers send, how many managers installed the app in the
last 30 days and how many run it, each with the time it was read. Counts below 10
are published as null. An entry whose homepage is a folder of a shared repository
...
is a read-only PostHog key (`query:read`, `project:read`); the project is found by
Appflare's public project key, or set the `POSTHOG_PROJECT_ID` variable. Stars come
from the GitHub GraphQL API with the workflow's own token. When a source fails, its
previous numbers are kept and it is marked `ok: false`. The workflow then takes the
newest site that `publish` or `nightly` built from `main` (their `catalog-site`
artifact), replaces only `stats.json`, and deploys it, so it can never roll the
index back; `publish` and `nightly` copy the live `stats.json` into every site they
build (`build-site --live-stats`), so they never remove the numbers. All three
deploy Pages one at a time (concurrency group `pages`). Until `publish` or
`nightly` has run once with this workflow in place, the stats run has no site to
update and fails with a message saying so.
