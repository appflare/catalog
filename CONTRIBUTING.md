...
apps/<slug>/icon.*, cover.png, screenshots/, MEDIA.md   images and their sources
...
5. Add the app's images next to the manifest (see "Images" below): an icon, a
   1200x630 `cover.png`, optional screenshots, and `MEDIA.md` saying where each
   one comes from.
...
apps/<slug>/screenshots/*.png      optional, at most 8, shown in file name order
apps/<slug>/MEDIA.md               where each image comes from, and its licence
```

`pnpm validate` requires the icon and the cover. Prefer the app's own logo and
social preview image when the upstream licence covers them; otherwise
`pnpm gen-media <slug>` makes a monogram icon and a cover from the name, summary
and repository, and records them in `MEDIA.md`. A screenshot's alt text comes from
its file name: `01-inbox-view.png` is read as "Inbox view". Every image must be
named in `MEDIA.md` with its source: an upstream file at a pinned commit (with the
licence and what was changed, such as scaling), or `scripts/gen-media.ts`. SVGs
may not contain scripts, event handlers, `foreignObject`, or links to other files.
...
pnpm gen-media <slug>... [--icon] [--cover] [--force]  # make a missing icon or cover
